...
export function OrderForm(props: OrderFormProps) {
  ...
  const handleSaveOrder = async (
    ...
    if (mode === "edit" && initialData?.id) {
      ...
      if (orderIndex !== -1) {
        ...
        if (newOrder.status === "Open") {
          notifyOrderUpdated(oldRequest, newOrder, currentUser);
        }
      }
    } else {
      ...
      if (hasMixed) {
        ...
        toast.success("Order split into 2: Kadar Muda & Kadar Tua");
    ...
    // Create notification for new order (only for new and duplicate modes)
    if (mode !== "edit") {
      const currentUser =
        sessionStorage.getItem("username") ||
        localStorage.getItem("username") ||
  ...
  useEffect(() => {
    if (initialData && mode !== "new") {
      ...
      // Restore atasNama for edit/duplicate modes
      if (mode !== "new" && initialData.atasNama) {
        setAtasNamaUsername(initialData.atasNama);
      }
  ...
  return (
    <div className="space-y-4 p-2 sm:p-0">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Order Will Be Split</AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm text-muted-foreground">
                <p>
                  Your order contains items from both{" "}
